 ...
 * insertAfterStation: method to insert a station in a location after a given
 * location - removeStation: write a method to remove a station at a given
 * location
 *
 *
 * Homework (due Monday 21 SEP 2020):
 *
 * - BONUS question: develop an interface based on class TrainRoute; name the
 * interface: Trains.
 *
 * - write a method called reverseRouteDisplay to print all stations along a
 * train route in reverse order, without modifying class TrainRoute's fields,
 * and without using any other data structures.
 *
 * - write a method called removeStationsBetween that will remove all stations
 * between a beginning and an ending location, e.g., in a Route like: A --> B
 * --> C --> D --> E --> F --> G --> H removeStationsBetween(C,G) shall reduce
 * the route to: A --> B --> C --> G --> H ie, the boundaries C and G are
 * preserved but everything in-between them has been removed.
 */
public class TrainRoute {
    ...
    // Displays a train line in reverse order
    public void displayReverse() {

        // Check if train line is null
        if (head == null) {
            System.out.println("The route is empty.");
        } else {
            Station currentStation = head; 
            ...
            // Calculate length of train line
            while (currentStation.next != null) {
                currentStation = currentStation.next;
                length++;
            }
            ...
            String outputPrefix = "Start of route:\t";
            // Run through the train route backwards, visiting every station
            for (int i = length; i >= 0; i--){
                ...
                // Change 'outputPrefix' for last station
                if (i == 0) {
                    outputPrefix = "End of route:\t";
                }

                // Print appropriate 'outputPrefix' along with current station
                System.out.println(outputPrefix + currentStation.city);

                // Set 'outputPrefix' after first station
                outputPrefix = "\t\t\t\t";
            }
            System.out.println();
        }
    ...
    // Remove stations between two indicated stations
    public boolean removeBetween(String startingC, String endingC) {

        // Status flag
        boolean success = false;

        // Check if train line is empty
        if (head != null) {

            // Check if starting and ending stations exist
            if (stationExists(startingC) && stationExists(endingC)) {
                Station current = head;

                // Run through train line
                while (current.next != null && !success) {

                    // Check if ending station is found before starting station
                    if (current.next.city.equals(endingC) && !current.next.city.equals(startingC)) {
                        System.out.println(endingC + " is located before " + startingC + " on the train line.\n");
                        success = true;
                        break;
                    }

                    // Find starting station
                    if (current.next.city.equals(startingC) && !success) {

                        // Check if there are stations between indicated stations
                        if (current.next.next.city.equals(endingC)) {
                            System.out.println("There are no stations between " + startingC + " and " + endingC + ".\n");
                            success = true;
                        } else {
                            while (!current.next.next.city.equals(endingC)) {
            ...
            System.out.println("The train line is empty.\n");
        }
        return success;
    }
    ...
    public static void main(String[] args) {
        ...
        TrainRoute emptyService = new TrainRoute();
        ...
        System.out.println("    ~ Reversed route");
        lincolnService.displayReverse();

        // Attempt to remove a non existing station from a populated route.
        lincolnService.removeStation("Rockford");

        System.out.println("    ~ 'removeBetween()' test on stations between Lincoln and Alton");
        lincolnService.removeBetween("Lincoln", "Alton");
        lincolnService.displayRoute();

        System.out.println("    ~ 'removeBetween()' test after stations between Lincoln and Alton are removed");
        lincolnService.removeBetween("Lincoln", "Alton");

        System.out.println("    ~ 'removeBetween()' test if ending station is before starting station");
        lincolnService.removeBetween("Alton", "Lincoln");

        System.out.println("    ~ 'removeBetween()' test on an empty train line");
        emptyService.removeBetween("Lincoln", "Alton");
